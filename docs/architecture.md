...
Medispeak is a model-agnostic clinical voice-scribe engine. The API layer never
names a model — it calls an **orchestrator** that composes **stages** (ASR,
structuring), each backed by a **provider adapter** resolved from runtime config.
A metering layer records usage per physical attempt and settles it against a
credit ledger.
...
| `Llm::ConfigResolver`     | Resolves a `Config` for a function using the most-specific `ModelAssignment` (Page → Account → System). Resolves the optional fallback model into `config.fallback`. |
| `Llm::DefaultConfigProvider` | ENV-based fallback `Config` (OpenAI defaults: `whisper-1` for ASR, `gpt-4o-mini` for structuring) when no `ModelAssignment` exists. |
...
| `Llm::Adapter` (+ `adapters/`) | Abstract base + concrete adapters (`OpenaiCompatible`, `Anthropic`). Each exposes `#transcribe` and `#structure`, and maps `Faraday::Error` to the `Llm::Error` hierarchy. |
...
| Component                  | Responsibility |
|----------------------------|----------------|
| `Scribe::Orchestrator`     | Runs the cascaded pipeline for one `ScribeSession`: ASR **once** (shared by all outputs), then each output independently, then rolls up the session status. |
...
1. **ASR once** — resolves the `asr` config, downloads the first attached audio
   blob into a Tempfile, runs `AsrStage`, and persists a `Transcript` on the
   session. If ASR fails, the session and **every** output are marked failed and
   it returns early (nothing to structure).
...
| Component                  | Responsibility |
|----------------------------|----------------|
| `Metering::UsageRecorder`  | Pure per-attempt meter. From a normalized `Llm::Result` it prices the call via `PriceBook`, snapshots unit prices, and persists a `UsageEvent`. |
| `Metering::PriceBook`      | Computes cost from versioned `ModelPrice` (per-million tokens) and `AudioModelPrice` (per-minute) tables, effective at a timestamp. Never raises on a missing price — it returns zeros so metering degrades gracefully. |
